...
class Search {
  ...
  events() {
    ...
    this.searchField.addEventListener("keyup", () => this.typeLogic());
  }
  typeLogic() {
    if (this.searchField.value != this.prevoiusValue) {
    ...
    this.prevoiusValue = this.searchField.value;
  }
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(universityData.root_url + "/wp-json/wp/v2/posts?search=" + this.searchField.value);
